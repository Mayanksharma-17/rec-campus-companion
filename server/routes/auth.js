...
router.post('/register', async (req, res) => {
  try {
    ...
    if (userIsHosteller) {
      userDesignation = matchedRegistry ? matchedRegistry.designation : 'Pearl Hostel';
...
// Endpoint: Instant Demo Switcher for Evaluation
router.post('/demo-switch', (req, res) => {
  const { email } = req.body;
  if (!email) {
    return res.status(400).json({ success: false, message: 'Email required for demo switch.' });
  }
  ...
  if (!user) {
    ...
    if (matchedRegistry) {
      user = {
        ...
        isClubMember: matchedRegistry.isClubMember,
        isStaff: matchedRegistry.isStaff,
        isAdmin: matchedRegistry.role === 'admin',
        clubsJoined: matchedRegistry.clubsJoined,
        designation: matchedRegistry.designation,
        pfpUrl: `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(matchedRegistry.name)}`,
        bio: 'Official REC Campus Companion User',
        phone: '+91 98765 43210'
      };
      db.users.push(user);
      ...
      return res.status(404).json({ success: false, message: `Demo user ${cleanEmail} not found.` });
    }
  }
  ...
  res.json({
    success: true,
    message: `Switched demo session to ${user.name} (${user.role.toUpperCase()})`,
    token,
    user: payload
  });
});
